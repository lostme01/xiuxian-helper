...
class Application:
    ...
    def setup_logging(self):
        ...
        print(f"日志系统配置完成。现在将同时输出到控制台和 {settings.LOG_FILE} 文件。")
    ...
    def load_plugins_and_commands(self, is_reload=False):
        if is_reload:
            ...
            core_handlers = {
                self.client._unified_event_handler,
                self.client._deleted_message_handler
            }
            if hasattr(self.client, 'client'):
                for handler, callback in list(self.client.client.list_event_handlers()):
                    if callback not in core_handlers:
        ...
        try:
            ...
            if self.redis_db.is_connected:
                try:
                    ...
                    format_and_log(LogType.SYSTEM, "身份注册",
                                   {'状态': '成功', '用户名': self.client.me.username, 'ID': self.client.me.id})
                except Exception as e:
    ...
    def register_command(self, name, handler, help_text="", category="默认", aliases=None, usage=None):
        ...
        command_data = {"name": name, "handler": handler, "help": help_text, "category": category, "aliases": aliases,
                        "usage": usage}
    ...
    async def reload_plugins_and_commands(self):
        ...
        try:
            ...
            # Then reload all plugins that might depend on the new settings
