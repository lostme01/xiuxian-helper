...
async def logic_clear_my_stall(client) -> str:
    ...
    返回一个操作摘要字符串。
    """
    format_and_log("TASK", "清理货摊", {'阶段': '开始查询'})
    ...
    if "尚未在万宝楼中上架任何物品" in reply_text:
        format_and_log("TASK", "清理货摊", {'阶段': '完成', '详情': '货摊为空'})
        return "✅ **清理完成**：您的货摊上没有任何物品。"
        
    # 3. 解析所有挂单ID
    listing_ids = re.findall(r'\*\*ID: (\d+)\*\*', reply_text)
    if not listing_ids:
        format_and_log("WARNING", "清理货摊", {'阶段': '解析失败', '原因': '无法从收到的回复中解析出ID'})
        return f"❓ **操作异常**：无法从货摊信息中解析出任何挂单ID。\n\n**游戏返回**:\n`{reply_text}`"
    
    format_and_log("TASK", "清理货摊", {'阶段': '解析成功', '待下架ID': str(listing_ids)})
    ...
    for item_id in listing_ids:
        try:
            ...
            format_and_log("ERROR", "清理货摊-单项失败", {'ID': item_id, '错误': str(e)})
    ...
    summary = "\n".join(report_lines)
    format_and_log("TASK", "清理货摊", {'阶段': '完成', '摘要': summary})
    return summary
