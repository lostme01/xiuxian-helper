...
async def _cmd_craft_gather(event, parts):
    ...
    if isinstance(plan, str): # 如果返回的是错误信息
        await progress_msg.edit(plan)
    ...
    for executor_id, materials in plan.items():
        ...
        for material, count in materials.items():
            try:
                await progress_msg.edit("\n".join(report_lines) + f"\n- 正在上架交易: `灵石*1` 换 `{material}*{count}`...")
                
                # 2.1 主导者上架
                list_command = f".上架 灵石*1 换 {material}*{count}"
                ...
                if "上架成功" in reply.raw_text and match:
                    ...
                    report_lines.append(f"  - `{material}*{count}` -> 挂单ID: `{listing_id}` (已通知)")
                    ...
                    report_lines.append(f"  - `{material}*{count}` -> ❌ **上架失败**")
                    await progress_msg.edit("\n".join(report_lines))
            
            except (CommandTimeoutError, Exception) as e:
                report_lines.append(f"  - `{material}*{count}` -> ❌ **上架异常**: `{e}`")
                await progress_msg.edit("\n".join(report_lines))
                continue
