...
class BaseExamSolver:
    ...
    async def _ask_gemini(self, question: str, options: dict) -> str | None:
        ...
        format_and_log(LogType.TASK, f"{self.log_module_name}: AI作答", {'状态': '开始请求Gemini'})
        try:
            # [重构] 调用新的、带模型降级的Gemini客户端
            response = await gemini_client.generate_content(prompt)
            answer = re.sub(r'[^A-D]', '', response.text.upper())
            if answer in options:
                format_and_log(LogType.TASK, f"{self.log_module_name}: AI作答", {'状态': '成功', '解析答案': answer})
                return answer
            else:
                format_and_log(LogType.TASK, f"{self.log_module_name}: AI作答", {'状态': '失败', '原因': 'AI返回了无效选项', '原始回复': response.text}, level=logging.WARNING)
                return None
        except Exception as e:
            format_and_log(LogType.TASK, f"{self.log_module_name}: AI作答", {'状态': '异常', '完整错误': repr(e)}, level=logging.ERROR)
            return None

    # [BUG修复] 修复异步调用问题
    async def _find_answer_in_db(self, question: str, options: dict) -> str | None:
        format_and_log(LogType.TASK, f"{self.log_module_name}: 数据库查询", {'问题': question})
        ...
        if stored_answer_text:
            for letter, text in options.items():
                if text == stored_answer_text:
                    format_and_log(LogType.TASK, f"{self.log_module_name}: 数据库查询", {'状态': '命中', '答案': f'{letter} ({text})'})
                    return letter
        format_and_log(LogType.TASK, f"{self.log_module_name}: 数据库查询", {'状态': '未命中'})
        return None
    ...
    async def _save_answer_to_db(self, question: str, answer_text: str):
        ...
        format_and_log(LogType.TASK, f"{self.log_module_name}: 答案入库", {'问题': question, '答案': answer_text})
    ...
    async def handler(self, event):
        ...
        format_and_log(LogType.TASK, f"流程启动: {self.log_module_name}", {'状态': '关键词匹配成功', '消息ID': event.id})
        ...
        if not (question and options and len(options) >= 4):
            format_and_log(LogType.TASK, f"流程中止: {self.log_module_name}", {'原因': '未能解析出有效的题目和选项', '原始文本': text}, level=logging.WARNING)
        ...
        format_and_log(LogType.TASK, f"{self.log_module_name}: 题目解析", {'问题': question, '选项': str(options), '是否轮到我': is_our_turn})
        ...
        if not answer_letter:
            ...
            if answer_letter:
                ...
                format_and_log(LogType.TASK, f"流程中止: {self.log_module_name}", {'原因': 'AI未能返回有效答案'})

        if is_our_turn and answer_letter:
            log_data = {'回复内容': f".作答 {answer_letter}", '答案来源': source}
            format_and_log(LogType.TASK, f"{self.log_module_name}: 准备作答", log_data)
            ...
            format_and_log(LogType.TASK, f"流程完成: {self.log_module_name}", {'状态': '已发送作答指令', '延时': f'{delay}秒'})
            
        elif is_our_turn:
            format_and_log(LogType.TASK, f"流程完成: {self.log_module_name}", {'状态': '放弃作答', '原因': '无法确定最终答案'})
        else:
            format_and_log(LogType.TASK, f"流程完成: {self.log_module_name}", {'状态': '无需作答', '原因': '未@本机'})
