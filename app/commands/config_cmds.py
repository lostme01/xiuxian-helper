...
HELP_DETAILS = {
    ...
    "redis配置": "查看或设置Redis连接参数。\n用法: `,redis配置 [<键> <值>]`\n可用键: `host`, `port`, `password`"
}

# ... (_cmd_sect_config, _cmd_garden_seed_config, _cmd_auto_delete_toggle 保持不变) ...

# *** 新增：Redis配置指令 ***
async def _cmd_redis_config(client, event, parts):
    # 查看模式
    if len(parts) == 1:
        ...
        status_text = (
            ...
            f"  - password: `{masked_pass}`\n"
            f"  - db: `{config.get('db')}`\n"
            f"  - xuangu_db_name: `{config.get('xuangu_db_name')}`\n"
            f"  - tianji_db_name: `{config.get('tianji_db_name')}`"
        )
    ...
    # 设置模式
    if len(parts) != 3:
        await event.reply(HELP_DETAILS["redis配置"], parse_mode='md')
        return
        
    key_to_set = parts[1].lower()
    value_to_set = parts[2]
    
    valid_keys = ['host', 'port', 'password']
    if key_to_set not in valid_keys:
        await event.reply(f"错误: 无效的配置键 '{key_to_set}'。可用键: `host`, `port`, `password`", parse_mode='md')
        return
    ...
    if key_to_set == 'port':
        try:
            ...
            await event.reply("错误: 端口号必须是一个数字。", parse_mode='md')
            return
    ...
    await update_setting(event,
        ...
        sub_key=key_to_set,
        value=value_to_set,
        success_message=f"✅ Redis `{key_to_set}` 已设置为 `{value_to_set}`。\n**请使用 `,重启` 指令以使新配置生效**"
    )
...
def initialize_commands(client):
    ...
    # *** 新增：注册新指令 ***
    client.register_admin_command("redis配置", _cmd_redis_config, HELP_DETAILS["redis配置"])
...
async def _cmd_sect_config(client, event, parts):
    ...
    if new_sect not in supported_sects:
        ...
        return
    await update_setting(event, root_key='sect_name', value=new_sect, success_message=f"宗门设置成功: **{new_sect}**。\n**请使用 `,重启` 指令**以加载新的宗门专属任务。")
async def _cmd_garden_seed_config(client, event, parts):
    ...
    await update_setting(event, root_key='huangfeng_valley', sub_key='garden_sow_seed', value=seed_name, success_message=f"小药园将优先播种 **{seed_name}**")
async def _cmd_auto_delete_toggle(client, event, parts):
    ...
    if len(parts) == 2 and parts[1] in ["开", "关"]:
        ...
        await update_setting(event, root_key='auto_delete', sub_key='enabled', value=new_status, success_message=f"**自动删除** 功能已 **{switch_action}**")
    else:
