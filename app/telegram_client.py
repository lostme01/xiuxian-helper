...
class TelegramClient:
    def __init__(self):
        ...
        # [重构] 统一的等待回复和编辑的中央分发系统
        self.pending_replies = {}  # key: sent_msg_id, value: {future, pattern}
        self.pending_edits = {}    # key: initial_reply_id, value: {future, pattern}
            ...
            try:
                ...
                if not target_group:
                    if future and not future.done():
                        future.set_exception(Exception("No target group specified for sending message."))
                    continue
    ...
    async def send_and_wait_for_edit(self, command: str, initial_pattern: str, final_pattern: str,
        ...
        try:
            ...
            if initial_reply_id:
                self.pending_edits.pop(initial_reply_id, None)
    ...
    async def _send_and_wait_for_response(self, command: str, final_pattern: str = ".*", timeout: int = None,
        ...
        try:
            ...
            if sent_message:
                self.pending_replies.pop(sent_message.id, None)
    ...
    async def _cache_chat_info(self):
        all_groups = set(
            settings.GAME_GROUP_IDS + ([settings.CONTROL_GROUP_ID] if settings.CONTROL_GROUP_ID else []) + (
                [getattr(settings, 'TEST_GROUP_ID', None)] if getattr(settings, 'TEST_GROUP_ID', None) else []))
    ...
    async def send_admin_notification(self, message: str):
        try:
            target = settings.CONTROL_GROUP_ID or self.admin_id
    ...
    async def _cancel_message_deletion(self, message: Message):
        ...
        if task := self.deletion_tasks.pop((message.chat_id, message.id), None):
            task.cancel()
    ...
    async def _unified_event_handler(self, event):
        """[重构] 统一处理新消息和编辑事件，并分发给等待中的任务。"""
        if not hasattr(event, 'message') or not hasattr(event.message, 'text') or not event.message.text:
            return
        ...
        await log_telegram_event(self, log_type, event)
        self.last_update_timestamp = datetime.now(pytz.timezone(settings.TZ))

        # 2. 分发给等待回复的任务 (NewMessage)
        if isinstance(event, events.NewMessage.Event) and event.is_reply:
            wait_obj = self.pending_replies.get(event.reply_to_msg_id)
            if wait_obj and not wait_obj['future'].done():
                if re.search(wait_obj['pattern'], event.text, re.DOTALL):
                    wait_obj['future'].set_result(event.message)

        # 3. 分发给等待编辑的任务 (MessageEdited)
        if isinstance(event, events.MessageEdited.Event):
            wait_obj = self.pending_edits.get(event.id)
            if wait_obj and not wait_obj['future'].done():
                if re.search(wait_obj['pattern'], event.text, re.DOTALL):
                    wait_obj['future'].set_result(event.message)
